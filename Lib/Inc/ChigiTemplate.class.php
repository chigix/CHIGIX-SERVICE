...
class ThinkTemplate {
     ...
     * 加载模板
     * @access public
     * @param string $tmplTemplateFile 模板文件：./Tpl/Default/Todo/index.html
    ...
    public function fetch($templateFile, $templateVar, $prefix = '') {
        /*@chiji 前端入口模板检查与部署*/
        $this->chiji->chijiTempCheck($templateFile); //检测模板文件是否存在，若不存在则自动生成
        //==原生引擎======================
    ...
    protected function parseInclude($content) {
        ...
        // 设定module列表存储器
        if (!APP_DEBUG) {
            Log::write('模板引擎加载错误，别名配置“' . APP_NAME . '/Conf/alias.php”中不能定义“ThinkTemplate”配置项！！', Log::ALERT);
        }
        if ($find) {
            for ($i = 0; $i < $find; $i++) {
                $include = $matches[1][$i];
                ...
                $file = $array['file'];
                /* @chiji 模块记录推送 */
                $this->chiji->moduleListPush($file);
                unset($array['file']);
    ...
    private function parseTemplateName($templateName) {
        ...
        foreach ($array as $templateName) {
            ...
            // 获取模板文件内容
            /* @chiji 针对非入口模板的加载编译及结果获取*/
            $parseStr .= $this->chiji->chijiTempCheck($templateName);
        }
        return $parseStr;
    }
